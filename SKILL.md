...
```powershell
cd $env:USERPROFILE\.claude\skills\wechat-claude-code
if (Test-Path node_modules) { "installed" } else { "not_installed" }
```

- 如果 `not_installed`：提示用户运行安装命令安装依赖，然后停止。
...
**Windows:**
```powershell
Get-ChildItem $env:APPDATA\wechat-claude-code\accounts\*.json -ErrorAction SilentlyContinue | Select-Object -First 1
```
...
所有命令的工作目录为 `~/.claude/skills/wechat-claude-code`（macOS/Linux）或 `$env:USERPROFILE\.claude\skills\wechat-claude-code`（Windows）。

| 命令 | 执行 | 说明 |
|------|------|------|
| setup | `npm run setup` | 首次安装向导：生成 QR 码 → 微信扫码 → 配置工作目录 |
| start | `npm run daemon -- start` | 启动守护进程（macOS: launchd, Linux: systemd/nohup） |
| start (win) | `npm run daemon:win -- start` | 启动守护进程（Windows: Task Scheduler） |
| stop | `npm run daemon -- stop` / `npm run daemon:win -- stop` | 停止守护进程 |
| restart | `npm run daemon -- restart` / `npm run daemon:win -- restart` | 重启守护进程 |
| status | `npm run daemon -- status` / `npm run daemon:win -- status` | 查看运行状态 |
| logs | `npm run daemon -- logs` / `npm run daemon:win -- logs` | 查看最近日志（tail -100） |
