...
/**
 * Strip potential API keys / tokens from error strings before logging or returning to user.
 */
function sanitizeError(text: string): string {
  return text
    ...
    .replace(/\b(ANTHROPIC_API_KEY|ANTHROPIC_AUTH_TOKEN)\s*[:=]\s*\S+/gi, "$1=<REDACTED>");
}

// ---------------------------------------------------------------------------
// Load Claude Code settings.json env vars (for cc-switch and other managers)
// ---------------------------------------------------------------------------

export function loadClaudeEnvFromSettings(): void {
  try {
    ...
    if (settings.env && typeof settings.env === "object") {
      for (const [key, value] of Object.entries(settings.env)) {
        if (typeof value === "string") {
          ...
          if (!existing) {
            ...
            logger.info("Loaded env from Claude settings", { key });
          }
        }
      }
      // Map ANTHROPIC_AUTH_TOKEN → ANTHROPIC_API_KEY for SDK compatibility
      if (settings.env["ANTHROPIC_AUTH_TOKEN"] && !process.env["ANTHROPIC_API_KEY"]) {
        ...
        logger.info("Mapped ANTHROPIC_AUTH_TOKEN to ANTHROPIC_API_KEY");
      }
    }
  } catch {
    // ignore missing or invalid settings.json
  }
}
...
function resolveGlobalClaudeCliPath(): string | undefined {
  try {
    ...
    const claudeBin = execSync(whichCmd, { encoding: 'utf8', ...(isWin ? { shell: true as any } : {}) }).split(/\r?\n/)[0].trim();
    if (!claudeBin) return undefined;

    // Resolve symlinks to get the actual file (cross-platform)
    let realBin: string;
    try {
      realBin = existsSync(claudeBin) ? require('fs').realpathSync.native(claudeBin) : claudeBin;
    } catch {
      realBin = claudeBin;
    }

    // On npm global installs, the binary itself may be cli.js
    ...
    const npmPrefixCmd = 'npm config get prefix';
    const npmPrefix = execSync(npmPrefixCmd, { encoding: 'utf8', ...(isWin ? { shell: true as any } : {}) }).trim();
    const npmCli = isWin
      ? join(npmPrefix, 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js')
      : join(npmPrefix, 'lib', 'node_modules', '@anthropic-ai', 'claude-code', 'cli.js');
...
export async function claudeQuery(options: QueryOptions): Promise<QueryResult> {
  ...
  logger.info("Starting Claude query", {
    ...
    envBaseUrl: process.env.ANTHROPIC_BASE_URL ? "set" : "not-set",
    envApiKey: process.env.ANTHROPIC_API_KEY ? "set" : "not-set",
    envModel: process.env.CLAUDE_MODEL || "not-set",
  });
  ...
  logger.debug("Built SDK options", {
    ...sdkOptions,
    systemPrompt: (sdkOptions as any).systemPrompt ? "claude_code+append" : "not-set",
    pathToClaudeCodeExecutable: (sdkOptions as any).pathToClaudeCodeExecutable || "not-set",
  });
      ...
      switch (message.type) {
        ...
        case "result": {
          ...
          if (rm.subtype === "success" && "result" in rm) {
            ...
            const sanitized = rm.errors.map(sanitizeError);
            errorMessage = sanitized.join("; ");
            logger.error("SDK returned error result", { errors: sanitized });
          }
          break;
        }
        case "system": {
