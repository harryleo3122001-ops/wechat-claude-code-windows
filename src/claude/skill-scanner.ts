...
function scanClaudeDir(claudeDir: string, skills: SkillInfo[], seen: Set<string>): void {
  // 1. skills/*/
 ...
 * 4. %LOCALAPPDATA%\Claude\skills\ (Windows)
 * 5. %APPDATA%\Claude\skills\ (Windows)
 */
export function scanAllSkills(): SkillInfo[] {
  ...
  if (process.platform === 'win32') {
    if (process.env.LOCALAPPDATA) claudeDirs.push(join(process.env.LOCALAPPDATA, 'Claude'));
    if (process.env.APPDATA) claudeDirs.push(join(process.env.APPDATA, 'Claude'));
  }
  claudeDirs.push(join(home, '.claude'));

  const skills: SkillInfo[] = [];
  const seen = new Set<string>();

  for (const claudeDir of claudeDirs) {
    scanClaudeDir(claudeDir, skills, seen);
  }
...
/**
 * Read a SKILL.md file and return its instruction content (everything after the frontmatter).
 */
export function loadSkillContent(skillPath: string): string | null {
  ...
  try {
    ...
    const instruction = content.slice(match[0].length).trim();
    return instruction || null;
  } catch {
    logger.warn(`Failed to read SKILL.md content: ${skillFile}`);
    return null;
  }
}
