...
import { scanAllSkills, formatSkillList, findSkill, loadSkillContent, type SkillInfo } from '../claude/skill-scanner.js';
...
export function handleUnknown(cmd: string, args: string): CommandResult {
  ...
  if (skill) {
    const skillContent = loadSkillContent(skill.path);
    let prompt: string;
    if (skillContent) {
      prompt = args
        ? `<skill_instructions>\n${skillContent}\n</skill_instructions>\n\nUser request: ${args}`
      ...
      prompt = args ? `Use the ${skill.name} skill: ${args}` : `Use the ${skill.name} skill`;
    }
