...
import { claudeQuery, type QueryOptions, loadClaudeEnvFromSettings } from './claude/provider.js';
import { loadConfig, saveConfig } from './config.js';
import { logger } from './logger.js';
import { DATA_DIR } from './constants.js';
import { MessageType, type WeixinMessage } from './wechat/types.js';
// Windows: set console code page to UTF-8 so Chinese characters render correctly
if (process.platform === 'win32') {
  try { spawnSync('chcp 65001', { shell: true, windowsHide: true }); } catch { /* ignore */ }
}
...
async function handleMessage(
  ...
  if (session.state === 'processing') {
    if (userText.startsWith('/clear')) {
      ...
      // Allow /status and /help to run concurrently without aborting
    } else {
      // Abort current query and let the new command through (includes skill commands)
      const ctrl = activeControllers.get(account.accountId);
      if (ctrl) { ctrl.abort(); activeControllers.delete(account.accountId); }
      session.state = 'idle';
      sessionStore.save(account.accountId, session);
